...
int anope_event_fjoin(char *source, int ac, char **av)
{
    ...
    /* these are used to generate the final string that is passed to anope's core */
    int nlen = 0;
    char nicklist[514];
...
void inspircd_cmd_global(char *source, char *buf)
{
    ...
    if (has_globopsmod)
        ...
        send_cmd(ud ? ud->uid : TS6SID, "SNONOTICE A :%s", buf);
...
int anope_event_capab(char *source, int ac, char **av)
{
    ...
    if (strcasecmp(av[0], "START") == 0) {
        ...
        if (has_joinfloodmod) {
            ...
            if (cbmi) {
                cbmi->getvalue = get_throttle;
                cbmi->csgetvalue = cs_get_throttle;

                myCbmodes['j'].flag = CMODE_j;
                myCbmodes['j'].flags = CBM_MINUS_NO_ARG | CBM_NO_MLOCK;
...
void inspircd_cmd_global_legacy(char *source, char *fmt)
{
    ...
    if (has_globopsmod)
        ...
        send_cmd(ud->uid, "SNONOTICE A :%s", fmt);
}
